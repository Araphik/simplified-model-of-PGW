...
#include <bearer.h> //нужен для работы с shared_ptr<bearer>

std::shared_ptr<pdn_connection> pdn_connection::create(uint32_t cp_teid,
                                                       boost::asio::ip::address_v4 apn_gw,
...
//Теперь default bearer устанавливается только один раз, чтобы предотвратить случайную перезапись. 
//Это важно для корректной логики и тестов, где default bearer создаётся через create_bearer.
...
pdn_connection::pdn_connection(uint32_t cp_teid,
                               boost::asio::ip::address_v4 apn_gw,
                               boost::asio::ip::address_v4 ue_ip_addr)
    : _apn_gateway(std::move(apn_gw)),
      _ue_ip_addr(std::move(ue_ip_addr)),
      _cp_teid(cp_teid) {}

// Добавляет bearer в таблицу bearers (по dp_teid)
void pdn_connection::add_bearer(std::shared_ptr<bearer> bearer) {
    _bearers[bearer->get_dp_teid()] = std::move(bearer);
}

// Удаляет bearer по dp_teid
